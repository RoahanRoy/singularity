// Screen 4: Human + AI Decision Layer (Console)
const { useState: useStateS4, useRef: useRefS4, useEffect: useEffectS4 } = React;
...
const sessionLog = [
  {
    ...
    body: (
      <p>Show me all high-conviction positions derived from post-earnings supplier dislocations in the last 72 hours.</p>
    ),
  },
  {
    who: "ai",
    ...
    body: (
      <div>
        <p>
          Surfacing <span className="h">7 positions</span> that meet the criteria. Three are derived from the
          TSM call (supplier-side); the remaining four trace to AVGO and the CHRG/EV cluster. Median conviction{" "}
  ...
  },
  {
    ...
    name: "Operator · K. Park",
    body: (
      <p>Reduce China-linked semiconductor exposure by 15%. Preserve idiosyncratic alpha. Run pre-trade through the new risk overlay.</p>
    ),
  },
  {
    who: "ai",
    ...
    body: (
      <div>
        <p>
          Modeled three execution paths. Path B preserves <span className="h">87%</span> of idiosyncratic alpha
          while reducing China-linked semi exposure from <span className="h">9.4%</span> to <span className="h">8.0%</span>{" "}
          (−15.0%). Pre-trade risk overlay passes. <span className="amber">28 sub-agents</span> participated; 2 dissented and requested human review.
        </p>
        <div className="quote">
          "Dissent · risk/topology-04 — the proposed unwind is correlated with our SOXX long; consider pacing over
          2 sessions to avoid book-internal hedging cost."
        </div>
        <div className="action-card">
  ...
  },
  {
    ...
    name: "Operator · K. Park",
    body: (
      <p>Spawn deeper forensic agents on the TSM filing anomaly. Limit at 8 agents, budget $480 in inference.</p>
    ),
  },
  {
    who: "ai",
    ...
    body: (
      <div>
        <p>
          ...
          (<span className="dim">$61 expected median spend</span>). Findings will route into the TSM memo as
          numbered sub-reports. ETA <span className="h">~7 minutes</span>.
        </p>
      </div>
    ),
  },
    ...
    <div className="chat">
      ...
      <div className="composer">
        ...
        <div className="hints">
          ...
          <span style={{ marginLeft: "auto", color: "var(--ink-4)" }}>voice <span className="kbd">⌘ V</span></span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="gov">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "6px 10px", fontFamily: "var(--mono)", fontSize: 11 }}>
...
function ConsoleScreen() {
  return (
    <div className="console">
      ...
      <Panel title="Governance & Telemetry" meta="" bodyClassName="tight">
        <Governance />
      </Panel>
    </div>
  );
