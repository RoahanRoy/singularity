...
// Timer is deliberately NOT unref'd — between cycles it is the only pending
// handle, so unref'ing would let Node exit silently (code 0) before the sleep
// resolves, killing the loop after a single pass. See ingest-held.ts.
